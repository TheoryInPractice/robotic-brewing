...
// NaiveDPSolverTest
//
TEST(NaiveDPSolverTest, SolveExact) {
  util::set_log_level(util::logging::LogLevel::NONE);

  for (int num_threads = 1; num_threads <= 2; ++num_threads) {
    for (int tc_opt = 0; tc_opt <= 1; ++tc_opt) {
      bool transitive_closure = tc_opt;
      {
        auto g = load_pace("src/test/resources/instances/001_tiny.gr").graph;
        auto solver = NaiveDPSolver(g, num_threads, false, false, transitive_closure);
...
TEST(NaiveDPSolverTest, SolveCorner) {
  int num_threads = 1;
  util::set_log_level(util::logging::LogLevel::NONE);
  ...
  for (int i = 101; i <= 103; ++i) {
    ...
    auto solver = NaiveDPSolver(g, num_threads);
