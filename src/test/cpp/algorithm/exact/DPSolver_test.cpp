...
TEST(DPSolverTest, SolveApproximate) {
  int num_threads = 2;
  util::set_log_level(util::logging::LogLevel::NONE);

  {
    auto g = load_pace("src/test/resources/instances/401_dp1.gr").graph;
    auto solver = DPSolver(g, num_threads, false, false, 0, 0.75);
  ...
  {
    auto g = load_pace("src/test/resources/instances/402_dp2.gr").graph;
    auto solver = DPSolver(g, num_threads, false, false, 0, .26);
    ...
    EXPECT_EQ(solver.set_epsilon(0), 0);
    EXPECT_EQ(solver.set_rho(0.25), 0.25);
    EXPECT_TRUE(solver.solve(4, 0, 0));
    EXPECT_EQ(solver.get_solution_weight(), 4);
    EXPECT_EQ(solver.get_solution_colors(), VI({1}));
    EXPECT_EQ(solver.get_solution(), VI({0, 3, 0}));
  }

  {
    ...
    EXPECT_TRUE(solver.solve(4, 0, 0));
    EXPECT_EQ(solver.get_solution_weight(), 12);
    EXPECT_EQ(solver.get_solution_colors(), VI({0, 1, 3}));
    EXPECT_EQ(solver.get_solution(), VI({0, 1, 3, 1, 0}));

    EXPECT_EQ(solver.set_epsilon(0), 0);
    EXPECT_EQ(solver.set_rho(0.5), 0.5);
    EXPECT_TRUE(solver.solve(4, 0, 0));
    EXPECT_EQ(solver.get_solution_weight(), 4);
    EXPECT_EQ(solver.get_solution_colors(), VI({0, 1}));
    EXPECT_EQ(solver.get_solution(), VI({0, 1, 0}));
...

  {
    ...
    EXPECT_TRUE(solver.solve(4, 0, 0));
    EXPECT_EQ(solver.get_solution_weight(), 16);
    EXPECT_EQ(solver.get_solution_colors(), VI({1, 2, 3}));
    EXPECT_EQ(solver.get_solution(), VI({0, 2, 3, 4, 3, 2, 0}));

    EXPECT_EQ(solver.set_epsilon(.5), .5);
    EXPECT_EQ(solver.set_rho(0.75), 0.75);
    EXPECT_TRUE(solver.solve(4, 0, 0));
    EXPECT_EQ(solver.get_solution_weight(), 15);
    EXPECT_EQ(solver.get_solution_colors(), VI({0, 1, 2}));
    EXPECT_EQ(solver.get_solution(), VI({0, 2, 3, 1, 0}));
  }
}
